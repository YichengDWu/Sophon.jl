...
function sample(pde::PDESystem, sampler::QuasiRandomSampler, strategy) where {device_type}
    ...
    pde_datasets = [QuasiMonteCarlo.sample(points, bound[1], bound[2], sampling_alg) .|>
                    Float32 for (points, bound) in zip(pde_points, pde_bounds)]

    boundary_datasets = [QuasiMonteCarlo.sample(points, bound[1], bound[2],
                                                sampling_alg) .|> Float32
...
function sample(pde::PDESystem, sampler::QuasiRandomSampler{P, B, SobolSample},
    ...
    pde_dataset = sobolsample(first(pde_points), first(pde_bounds)[1], first(pde_bounds)[2]) .|> Float32
    pde_datasets = fill(pde_dataset, length(pde_bounds))

    boundary_datasets = [sobolsample(points, bound[1], bound[2]) .|> Float32
                         for (points, bound) in zip(bcs_points, bcs_bounds)]
