...
symbolic_pde_loss_functions = [Sophon.build_symbolic_loss_function(pinnrep, eq;
    ...
    datafree_pde_loss_functions = Tuple([Sophon.build_loss_function(pinnrep, eq, pde_indvar, i)
                                         for (i, (eq, pde_indvar, integration_indvar)) in enumerate(zip(eqs,
                                                                                                        pde_indvars,
                                                                                                        pde_integration_vars))])

    datafree_bc_loss_functions = Tuple([Sophon.build_loss_function(pinnrep, bc, bc_indvar, i+1)
...
ff = eval(symbolic_pde_loss_functions[1])

derivative = Sophon.numeric_derivative
integral = pinnrep.integral
uu(cord, θ, phi) = phi(cord, θ)
p = pinnrep.default_p

ff(prob.p[1], prob.u0)

Zygote.gradient(θ -> sum(ff(prob.p[1], θ)), prob.u0)


(+).(
    (+).(
        (+).(
                (*).(-0.0001, derivative(phi, u, cord_u, [[0.0001220703125, 0.0], [0.0001220703125, 0.0]], 2, var"##θ#334")),
                (*).(5, (^).(u(cord_u, var"##θ#334", phi), 3))
            ),
        (*).(-5, u(cord_u, var"##θ#334", phi))
        ),
    derivative(phi, u, cord_u, [[0.0, 6.055454452393343e-6]], 1, var"##θ#334"))

(+).(
    (+).(
        (+).(
                (*).(-0.0001, derivative(phi, uu, prob.p[1], [[0.0001220703125, 0.0], [0.0001220703125, 0.0]], 2, prob.u0)),
                (*).(5, (^).(uu(prob.p[1], prob.u0, phi), 3))
            ),
        (*).(-5, uu(prob.p[1], prob.u0, phi))
        ),
    derivative(phi, uu, prob.p[1], [[0.0, 6.055454452393343e-6]], 1, prob.u0)
    )

(*).(-0.0001, derivative(phi, uu, prob.p[1], [[0.0001220703125, 0.0], [0.0001220703125, 0.0]], 2, prob.u0))

using StatsBase
Zygote.gradient(θ -> mean(abs2,
    begin
        let (x,t) =  (prob.p[1][[1], :], prob.p[1][[2], :])
            begin
                cord_u = vcat(x, t)
            end
        (+).(
        (+).(
            (+).(
                    (*).(-0.0001, derivative(phi, uu, cord_u, [[0.0001220703125, 0.0], [0.0001220703125, 0.0]], 2, θ)),
                    (*).(5, (^).(uu(cord_u, θ, phi), 3))
                ),
            (*).(-5, uu(cord_u, θ, phi))
            ),
        derivative(phi, uu, cord_u, [[0.0, 6.055454452393343e-6]], 1, θ)
        ) .- 0.0
        end
    end
...
fthat = :((cord, θ) -> begin
    ...
    (+).(
        (+).(
                (*).(-0.0001, derivative(phi, cord_u, [[0.0001220703125, 0.0], [0.0001220703125, 0.0]], 2, θ)),
                (*).(5, (^).(phi(cord_u, θ), 3))
            ),
        (*).(-5, uu(cord_u, θ, phi))
        ),
    derivative(phi, cord_u, [[0.0, 6.055454452393343e-6]], 1, θ)
    ) .- 0.0
    end
end
end)
...
Zygote.gradient(θ -> sum(fthis(prob.p[1], θ)), prob.u0)[1]


Zygote.gradient(θ -> sum(ff(prob.p[1], θ)), prob.u0)[1]
