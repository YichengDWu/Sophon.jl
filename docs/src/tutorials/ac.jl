...
pinnrep = (; eqs, bcs, domain, ps, defaults, default_p, additional_loss, depvars, indvars,
           dict_indvars, dict_depvars, dict_depvar_input, multioutput, init_params, phi,
           derivative, strategy, pde_indvars, bc_indvars, pde_integration_vars,
           bc_integration_vars, fdtype=Float64, eq_params=SciMLBase.NullParameters(),
           param_estim=false)
...
Zygote.gradient(θ -> sum(-0.0001 .*
                         Sophon.numeric_derivative(phi, uu, prob.p[1],
                                                   [
                                                       [0.0001220703125, 0.0],
                                                       [0.0001220703125, 0.0],
                                                   ], 2, θ) .^ 3 .*
                         Sophon.numeric_derivative(phi, uu, prob.p[1],
                                                   [[0.0, 6.055454452393343e-6]], 1, θ)),
                prob.u0)[1]
Zygote.gradient(θ -> sum(Sophon.numeric_derivative(phi, uu, prob.p[1],
                                                   [[0.0, 6.055454452393343e-6]], 1, θ)),
                prob.u0)[1]
