...
```@example convection
chain = BACON(2,1,8,1; hidden_dims = 32, num_layers=5)
```
...
```@example convection
discretization = PhysicsInformedNN(chain, QuasiRandomTraining(500; resampling = false, minibatch = 1); adaptive_loss = NonAdaptiveLoss(; bc_loss_weights = [500]))
